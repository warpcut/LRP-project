...
mcp_save = ModelCheckpoint("../../cq/model/model_80_nadam.h5", save_best_only=True, monitor='val_loss', mode='min')
...
pyplot.legend()
pyplot.savefig('../../cq/reports/report_model_80_nadam_acc.png')

pyplot.clf()
pyplot.plot(history.history['loss'], label='train_loss')
pyplot.plot(history.history['val_loss'], label='test_loss')
pyplot.legend()
pyplot.savefig('../../cq/reports/report_model_80_nadam_loss.png')
pyplot.clf()

scores = model.evaluate(valid_generator, steps=STEP_SIZE_VALID)
print("Scores on test set: loss=%s accuracy=%s" % tuple(scores))
f = open("../../cq/reports/report_model_80_nadam.txt", "w")
f.write("Scores on test set: loss=%s accuracy=%s" % tuple(scores))
f.close()

# Save trained model
model.save('../../cq/model/model_80_nadam.h5')
